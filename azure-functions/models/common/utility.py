...
def lstm_predict(df, ids_train, scaler_min, scaler_max, parking_model):
    ...
    out = parking_model(scaled, extra, feed_last_only=True)
...
def predict_from_numpy(id, ids_train, pred_length, model, pred_start=None):
    ...
    tmp_pred = np.concatenate([model, model], axis=-1)
